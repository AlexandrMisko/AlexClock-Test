...
taken_stamp[4] = resp.json()['data']['user']['edge_owner_to_timeline_media']['edges'][4]['node']['taken_at_timestamp']
taken_stamp[5] = resp.json()['data']['user']['edge_owner_to_timeline_media']['edges'][5]['node']['taken_at_timestamp']
taken_stamp[6] = resp.json()['data']['user']['edge_owner_to_timeline_media']['edges'][6]['node']['taken_at_timestamp']
rank = 3
for i in range(4,7):
    if taken_stamp[3] < taken_stamp[i]:
        taken_stamp[3] = taken_stamp[i]
        rank = i
...
if taken_at == store_at:
    ...
    if media_type == 2:
        ...
        items = resp.json()['items']['carousel_media']
        for item in items:
            if item['media_type'] == 1:
                contents += '<button type="button"><a href='+resp.json()['items'][0]['image_versions2']['candidates'][0]['url']+'>图片</a></button>'
            else:
                contents += '<button type="button"><a href='+resp.json()['items'][0]['video_versions'][0]['url']+'>视频</a></button>'
    yag.send(to='1586924294@qq.com', subject='AlexandrMisko更新啦！', contents='<h1>方式1（动态--复制下面链接到有Instagram登录状态的浏览器中打开）：</h1>'+url+'<h1>方式2（视频--直接打开即可）：</h1>'+contents)
    yag.close()
    print('发送邮件成功！')
